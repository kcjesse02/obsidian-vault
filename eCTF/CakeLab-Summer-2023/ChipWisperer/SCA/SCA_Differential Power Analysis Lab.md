For this lab, we will be breaking a simplified version of AES that XORs one byte with a secret key byte and uses that to lookup a value in an sbox table.
```python
sbox = [
